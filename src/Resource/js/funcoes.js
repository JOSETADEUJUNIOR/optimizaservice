...
const URL_API = window.PATH_URL = document.location.origin + '/optimizaserviceapi/src/Resource/api/';
const URL_IMG =  window.PATH_URL = document.location.origin + '/optimizaserviceapi/src/Resource/';
...
function GetTnkValue() {
    ...
    $.ajax({
        type: "POST",
        url: "session.php", // Substitua pelo caminho correto
        data: { token_data: j },
        success: function(response) {
            // Os dados podem ser usados posteriormente em relatórios
            console.log(response);
        }
    });
    // Adiciona a chamada AJAX aqui
    return JSON.parse(j);
...
function StatusVendaDesc(status)
{
    if (status==1) {
        return 'Orçamento';
    }else if(status==2){
...
/* function exibirDataHoraBr(dataHora) {
    if (dataHora == null || dataHora == "") {
        return "";
    } else {
        var dateTimeArray = dataHora.split(' ');
        var dataArray = dateTimeArray[0].split('-');
        var horaArray = dateTimeArray[1].split(':');

        var dataBr = dataArray[2] + '-' + dataArray[1] + '-' + dataArray[0]; // Mudança aqui
        var horaBr = horaArray[0] + ':' + horaArray[1];

        return dataBr + 'T' + horaBr; // Mudança aqui
    }
}
 */
