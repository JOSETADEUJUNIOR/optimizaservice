        ...
        if (NotificarCampos(id_form)) {
            ...
            $.ajax({
                ...
                success: function (dados_ret) {
                    ...
                    if (resultado == 1) {
                        MensagemGenerica("Tipo de serviço cadastrado com sucesso", "success");
                    } else {
                        MensagemErro();
                    }
                }
...
function DetalharTpServico() {
    ...
    var endpoint_cliente = "DetalharTpServico";
    var id = atob(encodedId);
    var dados = {
        endpoint: endpoint_cliente,
        tp_servico_id: decodedId,
    ...
    $.ajax({
        ...
        success: function (dados_ret) {
            ...
            $("#tp_servico_status").val(resultado.status);
            $("#id").val(resultado.id);
...
function EditarTpServico(id_form){
    if (NotificarCampos(id_form)) {
        let dadosAPI = GetTnkValue();
        if (!dadosAPI.tecnico_id) {
            Sair();
